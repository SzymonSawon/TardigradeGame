...
void player_init(PlayerControlSystem_Player *player, ResourceManagerSystem* resource_manager_system){
  ...
  player->sprite.sprite_index = resource_manager_load_texture_file(resource_manager_system, "sprites/meatchunk.png");
...
void player_control_system_update(PlayerControlSystem *sys, SpriteSystem *sprite_system){
  for(size_t i=0;i<sys->players.count;i++){
    sprite_push(sprite_system,
                    (Sprite_Info){.sprite_index = sys->players.items[i].sprite.sprite_index,
                                  .position = (Vector2){.x = sys->players.items[i].sprite.position.x, .y = sys->players.items[i].sprite.position.y}});
